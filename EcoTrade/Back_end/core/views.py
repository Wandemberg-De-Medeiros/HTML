...
def cadastro_view(request):
    if request.method == 'POST':
        ...
        USUARIOS_DB[email] = {"senha": senha, "tipo": tipo, "nome": nome}
...
def registro_creditos_view(request):
    if not request.session.get('logged_in') or request.session['user_info']['tipo'] != 'Produtor Rural':
        return redirect(reverse('login'))
...
def transacoes_view(request):
    ...
    context = {
        ...
        'user_tipo': request.session['user_info']['tipo']
    }
