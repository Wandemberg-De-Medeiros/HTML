...
from .models import Usuario, CreditoCarbono, Requisicao, Transacao, TIPO_USUARIO_CHOICES
            ...
            if credito_id and volume and preco_unitario:
                try:
                    ...
                    if volume_int > credito_a_vender.quantidade:
                        ...
                        # Atualiza o crédito para listado no marketplace
...
def requisicoes_transacao_view(request):
    ...
    requisicoes_pendentes = Requisicao.objects.filter(
        tipo_requisicao='Compra', 
        status='Pendente'
    ).select_related('usuario_origem', 'credito', 'credito__produtor')
