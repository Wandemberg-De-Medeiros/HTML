...
def historico_creditos_view(request):
    user, user_info = get_user_logged(request)
    if not user:
        return redirect(reverse('login')) # Redireciona se não estiver logado

    # Filtro base: Apenas créditos APROVADOS
    filtro_base = CreditoCarbono.objects.filter(status='Aprovado')

    if user.tipo == 'Produtor Rural':
        # Produtor vê apenas os créditos aprovados que ele registrou
        creditos = filtro_base.filter(produtor=user).select_related('produtor').order_by('-data_registro')
        titulo = "Meus Créditos Aprovados"
    else:
        # Administrador e Empresa Compradora veem todos os créditos aprovados
        creditos = filtro_base.select_related('produtor').order_by('-data_registro')
        titulo = "Histórico de Créditos Aprovados"
    ...
    context = {
        ...
        'active_page': 'historico_creditos',
        'pendencias': get_pendencias(),
        'titulo_pagina': titulo # Novo contexto para o título
    }
